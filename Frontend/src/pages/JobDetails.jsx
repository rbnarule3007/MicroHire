...
const JobDetails = ({ isClientView }) => {
    ...
    useEffect(() => {
        const fetchJobData = async () => {
            try {
                ...
                const url = `${API_BASE_URL}/api/jobs/${id}${user.userId ? `?freelancerId=${user.userId}` : ''}`;
                ...
                if (isFreelancer && user.userId) {
                    const fRes = await fetch(`${API_BASE_URL}/api/freelancers/${user.userId}`);
                    if (fRes.ok) {
                        setFreelancerProfile(await fRes.json());
                    }
                }

                const appRes = await fetch(`${API_BASE_URL}/api/applications/job/${id}`);
                if (appRes.ok) {
    ...
    const handleUpdateStatus = async (appId, newStatus) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/applications/${appId}/status?status=${newStatus}`, {
                method: 'POST'
            });
            if (res.ok) {
    ...
    const handleUpdateProgress = async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/jobs/${id}/progress`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const handleMarkComplete = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/jobs/${id}/complete`, {
                method: 'POST'
            });
