...
const Profile = () => {
    ...
    const fetchProfile = async (targetId) => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/freelancers/${targetId}/profile`);
    ...
    const handleAddProject = async (projectData) => {
        try {
            const url = editingProject
                ? `${API_BASE_URL}/api/freelancers/${profile.id}/projects/${editingProject.id}`
                : `${API_BASE_URL}/api/freelancers/${profile.id}/projects`;
    ...
    const handleSaveSkills = async (newSkills) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/freelancers/${profile.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleDeleteProject = async (projectId) => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/freelancers/${profile.id}/projects/${projectId}`, {
                method: 'DELETE'
            });
            if (res.ok) fetchProfile(profile.id || user.userId);
    ...
    const handleAddCertification = async (certData) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/freelancers/${profile.id}/certifications`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleDeleteCertification = async (certId) => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/freelancers/${profile.id}/certifications/${certId}`, {
                method: 'DELETE'
            });
            if (res.ok) fetchProfile(profile.id || user.userId);
